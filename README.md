...
pnpm brainbridge audit /path/to/notes --json
...
```bash
pnpm brainbridge audit [notes-path] [--out <dir>] [--json]
...
- `audit`: report what works and what degrades outside the source app. Use `--json` to print a share-safe JSON report to stdout for automation.
...
`brainbridge-report.json` is share-safe by default: local machine paths are redacted or represented relative to the scanned vault.

`export` writes a `vault/` bundle with Markdown notes, attachments, rewritten Markdown links, and Markdown fallbacks for Canvas and Bases. Raw `.canvas` and `.base` files are scanned as source inputs, but the portable bundle presents their generated Markdown fallbacks instead.
...
Review the generated report, JSON audit output, plugin dependencies, task/property indexes, graph files, and Markdown fallbacks.
