...
3. Run `pnpm brainbridge audit /path/to/notes` from the BrainBridge repo to identify what works across tools and what degrades. Use `pnpm brainbridge audit /path/to/notes --json` when an agent or script needs the share-safe report on stdout.
4. Run `pnpm brainbridge export /path/to/notes --out ./brainbridge-export` from the BrainBridge repo to create a portable bundle with Markdown notes, attachments, rewritten Markdown links, graph data, backlinks, task/property indexes, plugin reports, and static fallbacks. Raw Canvas and Bases files are represented by generated Markdown fallbacks in the portable vault.
